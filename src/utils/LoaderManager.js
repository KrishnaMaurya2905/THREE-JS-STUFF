// import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js'
// import { OBJLoader } from 'three/examples/jsm/loaders/OBJLoader.js'
// import { DRACOLoader } from 'three/examples/jsm/loaders/DRACOLoader.js'
// import { FontLoader } from 'three/examples/jsm/loaders/FontLoader.js'
// import { TextureLoader } from 'three'

// class LoaderManager {
//   #assets
//   #textureLoader = new TextureLoader()
//   #GLTFLoader = new GLTFLoader()
//   #OBJLoader = new OBJLoader()
//   #DRACOLoader = new DRACOLoader()
//   #FontLoader = new FontLoader()

//   constructor() {
//     this.#assets = {} // Dictionary of assets, can be different type, gltf, texture, img, font, feel free to make a Enum if using TypeScript
//   }

//   get assets() {
//     return this.#assets
//   }

//   set assets(value) {
//     this.#assets = value
//   }

//   /**
//    * Public method
//    */

//   get(name) {
//     return this.#assets[name]
//   }

//   load = (data) =>
//     new Promise((resolve) => {
//       const promises = []
//       for (let i = 0; i < data.length; i++) {
//         const { name, gltf, texture, img, font, obj } = data[i]

//         if (!this.#assets[name]) {
//           this.#assets[name] = {}
//         }

//         if (gltf) {
//           promises.push(this.loadGLTF(gltf, name))
//         }

//         if (texture) {
//           promises.push(this.loadTexture(texture, name))
//         }

//         if (img) {
//           promises.push(this.loadImage(img, name))
//         }

//         if (font) {
//           promises.push(this.loadFont(font, name))
//         }

//         if (obj) {
//           promises.push(this.loadObj(obj, name))
//         }
//       }

//       Promise.all(promises).then(() => resolve())
//     })

//   loadGLTF(url, name) {
//     return new Promise((resolve) => {
//       this.#DRACOLoader.setDecoderPath('https://www.gstatic.com/draco/v1/decoders/')
//       this.#GLTFLoader.setDRACOLoader(this.#DRACOLoader)

//       this.#GLTFLoader.load(
//         url,
//         (result) => {
//           this.#assets[name].gltf = result
//           resolve(result)
//         },
//         undefined,
//         (e) => {
//           console.log(e)
//         }
//       )
//     })
//   }

//   loadTexture(url, name) {
//     if (!this.#assets[name]) {
//       this.#assets[name] = {}
//     }
//     return new Promise((resolve) => {
//       this.#textureLoader.load(url, (result) => {
//         this.#assets[name].texture = result
//         resolve(result)
//       })
//     })
//   }

//   loadImage(url, name) {
//     return new Promise((resolve) => {
//       const image = new Image()

//       image.onload = () => {
//         this.#assets[name].img = image
//         resolve(image)
//       }

//       image.src = url
//     })
//   }

//   loadFont(url, name) {
//     // you can convert font to typeface.json using https://gero3.github.io/facetype.js/
//     return new Promise((resolve) => {
//       this.#FontLoader.load(
//         url,

//         // onLoad callback
//         (font) => {
//           this.#assets[name].font = font
//           resolve(font)
//         },

//         // onProgress callback
//         () =>
//           // xhr
//           {
//             // console.log((xhr.loaded / xhr.total) * 100 + '% loaded')
//           },

//         // onError callback
//         (err) => {
//           console.log('An error happened', err)
//         }
//       )
//     })
//   }

//   // https://threejs.org/docs/#examples/en/loaders/OBJLoader
//   loadObj(url, name) {
//     return new Promise((resolve) => {
//       // load a resource
//       this.#OBJLoader.load(
//         // resource URL
//         url,
//         // called when resource is loaded
//         (object) => {
//           this.#assets[name].obj = object
//           resolve(object)
//         },
//         // onProgress callback
//         () =>
//           // xhr
//           {
//             // console.log((xhr.loaded / xhr.total) * 100 + '% loaded')
//           },
//         // called when loading has errors
//         (err) => {
//           console.log('An error happened', err)
//         }
//       )
//     })
//   }
// }

// export default new LoaderManager()



import * as THREE from 'three';
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import { FontLoader } from 'three/examples/jsm/loaders/FontLoader.js';
import { OBJLoader } from 'three/examples/jsm/loaders/OBJLoader.js';

class LoaderManager {
  static #instance;
  #assets = {};

  constructor() {
  if (LoaderManager.#instance) {
    return LoaderManager.#instance;
  }
  LoaderManager.#instance = this;

  // Expose these handlers to allow UI feedback
  this.onProgress = null;
  this.onLoad = null;

  // Setup internal LoadingManager
  this.loadingManager = new THREE.LoadingManager();

  // Hook into progress and load callbacks
  this.loadingManager.onProgress = (url, itemsLoaded, itemsTotal) => {
    const percent = Math.floor((itemsLoaded / itemsTotal) * 100);
    if (this.onProgress) this.onProgress(percent);
  };

  this.loadingManager.onLoad = () => {
    if (this.onLoad) this.onLoad();
  };

  this.gltfLoader = new GLTFLoader(this.loadingManager);
  this.textureLoader = new THREE.TextureLoader(this.loadingManager);
  this.fontLoader = new FontLoader(this.loadingManager);
  this.objLoader = new OBJLoader(this.loadingManager);
}


  static getInstance() {
    return new LoaderManager();
  }

  // Access loaded assets
  get assets() {
    return this.#assets;
  }

  // Clear loaded assets (optional utility)
  clear() {
    this.#assets = {};
  }

  // Load GLTF
  loadGLTF(path, name) {
    return new Promise((resolve, reject) => {
      this.gltfLoader.load(
        path,
        (gltf) => {
          this.#assets[name].gltf = gltf;
          resolve(gltf);
        },
        undefined,
        (err) => reject(err)
      );
    });
  }

  // Load Texture
  loadTexture(path, name) {
    return new Promise((resolve, reject) => {
      this.textureLoader.load(
        path,
        (texture) => {
          this.#assets[name].texture = texture;
          resolve(texture);
        },
        undefined,
        (err) => reject(err)
      );
    });
  }

  // Load Image
  loadImage(path, name) {
    return new Promise((resolve, reject) => {
      const image = new Image();
      image.onload = () => {
        this.#assets[name].image = image;
        resolve(image);
      };
      image.onerror = (err) => reject(err);
      image.src = path;
    });
  }

  // Load Font
  loadFont(path, name) {
    return new Promise((resolve, reject) => {
      this.fontLoader.load(
        path,
        (font) => {
          this.#assets[name].font = font;
          resolve(font);
        },
        undefined,
        (err) => reject(err)
      );
    });
  }

  // Load OBJ
  loadObj(path, name) {
    return new Promise((resolve, reject) => {
      this.objLoader.load(
        path,
        (obj) => {
          this.#assets[name].obj = obj;
          resolve(obj);
        },
        undefined,
        (err) => reject(err)
      );
    });
  }

  // Main Load Function
  load(data) {
    if (!Array.isArray(data) || data.length === 0) {
      console.warn('[LoaderManager] No valid data provided to load().');
      return Promise.resolve([]);
    }

    const promises = [];

    for (let i = 0; i < data.length; i++) {
      const { name, gltf, texture, img, font, obj } = data[i];

      if (!name) {
        console.warn(`[LoaderManager] Asset at index ${i} is missing a 'name'. Skipping.`);
        continue;
      }

      if (!this.#assets[name]) {
        this.#assets[name] = {};
      }

      if (gltf) promises.push(this.loadGLTF(gltf, name));
      if (texture) promises.push(this.loadTexture(texture, name));
      if (img) promises.push(this.loadImage(img, name));
      if (font) promises.push(this.loadFont(font, name));
      if (obj) promises.push(this.loadObj(obj, name));
    }

    return Promise.all(promises);
  }
}

export default LoaderManager.getInstance();
